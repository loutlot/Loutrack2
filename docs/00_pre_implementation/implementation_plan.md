...
- Charuco 内部較正 + wand 外部較正ワークフロー整備
- Host 主導の収録制御（露光/FPS配布、mask init、start/stop）を整備
- 受動発見（UDP受信）+ inventory 併用で制御対象カメラを確定
- 映像なし GUI（同期スライダー）で複数Pi設定を一括調整
...
9. 較正スクリプト実装(Charuco)
10. 外部較正スクリプト実装(wand)
11. Host 制御GUI実装（同期スライダー + wandセッション操作）
12. 5剛体化 + SteamVR 出力
