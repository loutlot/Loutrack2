...
marker loss の診断では `--marker-detection-model` を使う。既定の `iid_dropout` は旧来の独立ランダム dropout で、sensor / exposure / projection size と無関係に non-anchor marker を消す。Pi 実ログのように false blob が少なく SNR が高い前提を試す場合は `pi_snr` を使う。`pi_snr` は `--marker-dropout` を最大ストレス量として扱い、projected equivalent diameter が `--centroid-noise-reference-diameter-px` より小さい blob だけ低SNRリスクを連続的に上げる。active anchor は遮蔽以外のランダム検出落ちから外す。

rigid shape のプリント前比較では `five_rigid_body_mesh_lite_v1` を使う。この scenario は deterministic marker drop を入れず、身長 1.7m の参考姿勢に合わせ、HMD/顔面、胸中央、腹部/腰、左右足の表面搭載位置を simplified human pose として使う。各 rigid の local `+Z` は capsule/sphere 表面の外向き法線に合わせ、`tools/cad/rigid_body_fixture.py` の CAD fixture 座標にある base 面から tracking centroid までの高さを使って marker cluster を表面外側に置く。`--mesh-lite-occlusion body_capsules` を追加すると、full human mesh ではなく、head sphere、torso capsule、left/right leg capsule の簡易 body volume を camera-to-marker ray に当て、ray が marker 手前で body volume を横切る場合だけ追加で marker を隠す。表面搭載 marker 自身の endpoint 近傍だけは除外し、far-side mount は自身の body volume でも遮蔽され得る。現5blob形状と `--rigids` で渡した候補形状を同じ scenario / seed / camera rig で回し、`passive_stabilization_v2.shape_observability_summary_by_rigid` の `two_ray_markers_ge_4_ratio`、`two_ray_markers_ge_3_ratio`、`visible_markers_ge_4_ratio`、`visible_cameras_ge_2_ratio`、`marker_views_ge_8_ratio` を比較する。これは shape freeze 前の入口であり、最終的な人体メッシュ再現ではない。従来の `five_rigid_body_occlusion_v1` / `five_rigid_body_occlusion_relaxed_v1` は deterministic marker drop も重ねる tracking stress として扱う。

GUI live の object gating は、同一 camera 内で近接している blob assignment を曖昧として落とす。既定の `object_gating_ambiguous_blob_min_separation_px` は `0.60px`、`object_gating_ambiguous_blob_diameter_overlap_ratio` は `0.30`、`object_gating_ambiguous_marker_assignment_min_margin_px` は `0.29px`。これは close marker / cross-rigid 近接時に marker swap や wrong owner を triangulation 入力へ入れないための guard で、`rigid_stabilization_overrides` から PDCA 用に変更できる。

実ログ replay でも同じ guard は `--object-gating-ambiguous-blob-min-separation-px`、`--object-gating-ambiguous-blob-diameter-overlap-ratio`、`--object-gating-ambiguous-marker-assignment-min-margin-px` で変更できる。min separation に `0` を指定すると close-blob ambiguity drop を無効にできるため、採用前確認では OFF/ON 比較に使う。
