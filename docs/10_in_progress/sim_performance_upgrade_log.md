...
| `five_rigid_dance_swap_red_v1` | wrong / confusion を意図的に起こす red stress | pass を期待しない。混同の検出力と悪化量を見る。 |
...
| stress knobs | occlusion、false blob、dropout、alias blob、足交差など。 |
...
`five_rigid_dance_swap_red_v1` は pass 目的ではない。wrong/confusion を作れるか、検出できるかを見る。
...
| pair p95 | `23.125ms` |
| rigid p95 | `16.287ms` |
| geometry p95 | `5.180ms` |
| valid min | `0.000` |
| wrong/confusion/jump | `770/30/0` |
| max pose jump | `0.062m` |
| max pose flip | `60.4deg` |
