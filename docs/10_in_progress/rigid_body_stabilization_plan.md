...
5. Shadow diagnostics として `reacquire_guard.would_reject`, `reason`, `thresholds`, `score` を出す。
6. Shadow replay で効果が確認できたら、feature flag 付きで `REACQUIRE` mode のみ enforcement を有効化する。
7. Enforcement で reject した場合は accepted pose を commit せず、invalid pose として tracker に流す。`invalid_reason = "reprojection_guard_rejected"` を残す。
...
7. Phase 4.5: reacquire candidate guard。まず shadow mode、次に feature flag 付き enforcement。
