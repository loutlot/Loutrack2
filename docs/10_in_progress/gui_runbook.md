...
- wand の 4 点マーカー（長辺中点を含む）が破損していない
...
- `session_meta.pose_validation` に実測の reprojection / positive depth / triangulation angle が入る

raw similarity solve の確認項目:

- `session_meta.pose_validation.median_reproj_error_px`
- `session_meta.pose_validation.positive_depth_ratio`
- `session_meta.pose_validation.triangulation_angle_deg_p50`
- `session_meta.excluded_camera_ids`

補足:

- 観測不足または初期化失敗のカメラは、処理全体を止めずに除外される
- 除外情報は出力 JSON の `session_meta.excluded_camera_ids` と `session_meta.excluded_camera_reasons` に残る
- 品質確認には `session_meta.pose_validation` と `session_meta.optimizer` を使う
...
- `session_meta.wand_metric_validation` に floor / metric 側の評価が入る
...
`calibration/calibration_extrinsics_v1.json` が出ているのに台数が少ない場合:
...
- 再マスク実施の有無
- tracking 品質の所感
- `pose_validation` / `wand_metric_validation` と `optimizer.iterations` の異常有無
