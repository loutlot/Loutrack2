...
`linuxptp` は install するだけでは足りません。最低限、次が整っている必要があります。

- `pi-cam-01` で Grandmaster 側の `ptp4l` / `phc2sys` が起動している
- 他 Pi で client 側の `ptp4l` / `phc2sys` が起動している
- `pmc GET TIME_STATUS_NP` が実行できる
- `capture.py` の `ping` で `clock_sync.status` が `locked` または短時間の `degraded` に収まる

初期セットアップは `sudo ./src/pi/setup_ptp.sh master` または `sudo ./src/pi/setup_ptp.sh slave` を使うと、`linuxptp` 導入、unit 配置、service 起動まで 1 コマンドで入ります。既定は `software` timestamping です。
...
この結果で `master_offset` が読めない、または `gmPresent false` が続く場合は、`linuxptp` を入れただけで設定や常駐が未完了です。`capture.py` 側では自動設定しないので、先に PTP 側を直します。
