...
- `pose.solve_summary` に `usable_rows` / `complete_rows` / `median_reproj_error_px` / `p90_reproj_error_px` が入る
...
- 再マスク実施の有無
- tracking 品質の所感
- `pose.solve_summary` の `usable_rows` / `complete_rows` / reprojection 指標の異常有無
