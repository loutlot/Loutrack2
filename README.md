...
- Host GUI の `selected_camera_ids` は UI 選択として保持しつつ、camera status / calibration workflow / preview 切替は全 discovered camera を対象にするよう修正
- `loutrack_gui_settings.json` の自動巻き戻りを抑止し、`GET /api/settings` や `POST /api/command` / `POST /api/config` では `ui.selected_camera_ids` を書き戻さないよう修正
...
- `pi-cam-01` は `setup_ptp.sh master` により boot 時に一度だけ NTP bootstrap を行ってから PTP を開始
