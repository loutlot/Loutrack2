...
- `src/camera-calibration/extrinsics_scale.py` の scale 推定を wand 全点の similarity fit + 安定辺長比のハイブリッドへ変更し、外れ値除去と `expected_baseline_m` による候補選別を追加
...
- `Apply Wand Scale/Floor` の既定 `max_wand_metric_samples` を `16` に引き上げ、GUI から `expected_baseline_m` とあわせて wand PnP / joint BA 経路を使えるように更新
- `wand_metric` の 4点対応は `solvePnP` 再投影誤差ベースの permutation 探索で再ラベルするよう変更し、画像平面上の距離ヒューリスティクス依存を下げた
