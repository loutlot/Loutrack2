...
- v1 互換、wand metric、Apply Wand Scale/Floor、world bake-in を現行 extrinsics workflow から外し、GUI を `Pose Capture -> Generate Extrinsics` の 2 ステップへ簡素化
- `src/host/geo.py` / `src/host/view_extrinsics_3d.py` / `src/host/wand_gui.py` を v2 reader・v2 summary・pose-only viewer に更新
- `tests/test_extrinsics_pose_v2.py` / `tests/test_extrinsics_gui_v2.py` / `tests/test_geo_loader_v2.py` / `tests/test_view_extrinsics_3d_v2.py` を追加し、old wand extrinsics テストを整理
...
- `src/host/view_extrinsics_3d.py` を現行 extrinsics schema に合わせて更新し、`pose_validation` / `wand_metric_validation` の要約を 3D viewer 注記へ表示するよう修正
- `src/host/view_extrinsics_3d.py` で metric/floor 適用済み extrinsics の `wand.points_mm` を world 座標へ重ね描きできるようにし、camera frustum / floor / wand を同時に確認可能にした
- `src/camera-calibration/calibrate_extrinsics.py` を 2-pass pose solve（pass1初期推定 -> 幾何ゲート -> pass2再初期化+BA）へ更新し、`sample_selection.gating` に hard reject / soft gate の統計と pass1/pass2 validation 比較を記録
- `src/camera-calibration/extrinsics_ba.py` に `gate_weight` 乗算を追加し、境界サンプルを捨てずに重みで寄与を下げる hybrid gating を有効化
- `src/camera-calibration/extrinsics_scale.py` と `apply_wand_scale_floor()` を canonical pose + `session_meta.to_world_coords_matrix` 分離保存へ変更し、camera pose を world に焼き込まない schema へ移行
- `src/host/geo.py` の `CalibrationLoader.apply_extrinsics(..., apply_world_transform=True)` で `camera_pose_frame=metric_camera` の場合だけ `to_world_coords_matrix` を適用する互換ローダーを追加
- `src/host/view_extrinsics_3d.py` / `src/host/wand_gui.py` を新 schema に追従し、viewer で world transform 適用表示、GUI apply summary で `camera_pose_frame` と `world_transform_applied` を返すよう更新
- `tests/test_wand_extrinsics.py` / `tests/test_wand_gui.py` を更新し、幾何ゲート、`to_world_coords_matrix` 保存、loader の world transform 適用、GUI summary の新フィールドを回帰テスト化
