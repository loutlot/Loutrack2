...
`Generate Extrinsics` / CLI の similarity solve は、既定で BA 入力を `80` サンプルへ等間隔間引きします。長い pose capture でも GUI が固まりにくいようにするためです。
...
- 実機観察に合わせて 4点wand の幾何前提を更新（`short` を off-line 点、`elbow-mid-long` を同一直線）し、`WAND_POINTS_MM` と対応付けロジックを修正
- wand座標は `WAND_OUTER_SHORT_MM=182.0` / `WAND_OUTER_LONG_MM=257.0` を中心間距離として直接使用し、`mid` は `long/2`（128.5mm）へ更新
- `schema/README.md` の `calibration_extrinsics_v1` 例を新しい wand 座標（`182 / 128.5 / 257`）へ更新
...
- `src/pi/capture.py` の `pose_capture` blob 選択に area だけでなく image center と前フレーム整合を入れ、単点追従を安定化
