...
同期スライダー（exposure/gain）と `ping` / `mask_start` / `pose_capture` / `wand_metric_capture` / `Generate Extrinsics` をブラウザから操作する最小 GUI。fps は 118 固定で、pair window は half-frame bound の `4237us` に固定されます。tracking 側の host sync-quality dashboard は廃止され、tracking と extrinsics capture の stream start は PTP/epoch `start_at_us` による countdown start を使います。
